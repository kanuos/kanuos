...
export const InputField = ({name, value='', inputMode=true, getData, constraints, setEditMode, index, total}) => {
        ...
        return (
            ...
            max-w-lg md:min-w-[20rem] group flex items-start">
                <div className="flex flex-col items-start w-full gap-y-1">
                    ...
                    <p className={"mt-1 px-4 tracking-wider flex items-start " + (name === 'message' && ' whitespace-pre-line ' ) + (name === 'name' && ' capitalize ' ) + (name === 'email' && ' lowercase ' )}>
                        <small>
                            {field}
                        </small>
                    </p>
                </div>
                <button className="inline-block px-0.5 invisible group-hover:visible hover:text-primary transition-all" onClick={() => setIsInputMode(true)}>
                <MdOutlineEdit />
                </button>
            </article>
        )
    ...
    return (
        ...
        <AnimatePresence>
            <motion.ul 
                ...
                initial='hide'
                animate={(focused || canSubmit || field.trim().length > 0) ? 'show' : 'hide'}
            ...
            {Object.values(constraints).map(({message, check}) => {
                ...
                return (
                    ...
                    animate='show'
                    key={message} className={"text-xs flex items-center justify-start gap-x-1 " + (isValid ? 'text-secondary line-through' : 'text-primary')}>
                    {isValid ? <MdCheckCircle /> : <MdOutlineErrorOutline />} 
                    <small className="font-semibold tracking-wide">
                        {message}
                    </small>
                </motion.li>
            )})}
...
// TODO: submit message to backend -> show sucess message -> show redirect options