...
export const JSONEditor = ({
  ...
  function handleDeleteChapter(chapter, fieldKey) {
    ...
    const newChapters = state[fieldKey].filter(
      (el) => el.index !== chapter.index
    );
  ...
  useEffect(() => {
    ...
    const currentStateInSessionStorage = JSON.parse(
      sessionStorage.getItem(JSON_EDITOR_STATE)
    );
  ...
  return (
    <div
      key={type}
      className="section h-full w-full flex flex-col items-center justify-center select-none"
    >
      <section className="flex flex-col items-stretch justify-start gap-4 max-w-4xl w-full mx-auto">
        ...
        {CONTENT_TYPE[type]?.fields?.map((field, i) => {
          return (
            <div
              id={field.key}
              key={i}
              className="group flex flex-col items-stretch mb-6 relative after:absolute after:-bottom-10 after:-z-10 z-10 after:left-4 after:bg-dark after:h-10 after:w-0.5 after:last-of-type:hidden"
            >
              {field.type === "array" && (
                <ArrayField
                  name={field.key}
              ...
              {["tags"].includes(field.type) && (
                <ConstArrField name="tags" array={tags} />
              )}
              {field.type === "slug" && (
                <SlugField text={state?.title} getSlug={getSlug} />
              )}
              {field.type === "string" && (
                <StringField
                  name={field.key}
              ...
              {["obj"].includes(field.type) && (
                <ObjectStepInput
                  ...
                  init={state?.[field.key]}
                  setValue={({ key, value }) =>
                    handleStateUpdate({ k: key, v: value })
                  }
                  name={field.key}
                />
              )}
