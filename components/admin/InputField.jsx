...
export const StringField = ({ name, value = "", setValue }) => {
  return (
    <form className="flex flex-col items-start justify-start gap-2 bg-light p-4 filter drop-shadow-xl rounded-md">
      <label htmlFor={name} className="text-xs capitalize font-semibold">
        {name}
      </label>
      <Textarea
        id={name}
        ...
        className={
          "resize-none w-full p-1.5 text-xs peer outline-none focus:outline-none border-2 rounded overflow-hidden border-transparent focus:border-current " +
          (value.trim().length === 0 ? "text-primary" : "text-dark")
        }
        value={value}
...
export const ConstArrField = ({ name, array }) => {
  return (
    <section className="flex flex-col items-start justify-start gap-2 bg-light p-4 filter drop-shadow-xl rounded-md">
      ...
      <article className="flex flex-wrap gap-4 w-full p-1.5 text-xs uppercase text-secondary font-semibold">
        {array.map((el) => (
          <small key={el._id}>{el.tag}</small>
        ))}
      </article>
    </section>
  );
...
export const SlugField = ({ text = "", getSlug }) => {
  ...
  return (
    <section className="flex flex-col items-start justify-start gap-2 bg-light p-4 filter drop-shadow-xl rounded-md">
      ...
      <article
        className={
          "flex flex-wrap gap-4 w-full p-1.5 text-xs peer outline-none focus:outline-none border-2 rounded focus:border-current font-semibold " +
          (text.length > 0 ? "text-secondary" : "text-primary")
        }
      >
        {slug}
      </article>
...
export const ArrayField = ({ name, init = [], getData }) => {
  ...
  function handleAddToList(e) {
    ...
    try {
      ...
      if (Array.from(init).includes(sanitizedItem)) {
  ...
  return (
    <div className="flex flex-col w-full bg-light p-4 filter drop-shadow-xl rounded-md">
        ...
        <section className="w-full bg-light p-4 rounded-md filter drop-shadow-xl">
          <label
            ...
            className={
              ...
              (init.length > 0 ? "text-dark" : "text-primary")
            }
          >
            {name} ({init.length})
          </label>
      ...
      {init.length > 0 && (
        <>
          <small className="p-2 mt-2 font-semibold text-dark capitalize">
            {name} items
          </small>
        </>
      )}
      <ul
        key={init.length}
        className="flex flex-col items-start justify-start list-inside list-disc"
      >
        {init.map((el, i) => (
          <li
            key={i}
            className="p-2 text-xs"
            onClick={() => handleRemoveFromList(el)}
          >
            <small>{el.text}</small>
          </li>
        ))}
      ...
      {previewMode && (
        <div className="flex flex-col items-center min-h-[60vh] justify-center gap-4 relative rounded-md overflow-hidden">
          <Image
            ...
            alt="image preview"
            className="w-full h-full object-cover my-4 block shadow-lg"
          />
          <button
      ...
      {!previewMode && (
        <article className="flex flex-col items-start justify-start">
          ...
          <section className="w-full my-2">
            <label
              htmlFor="text"
              className="text-xs capitalize font-semibold text-secondary"
            >
              Img URL
            </label>
            <Textarea
...
export const ObjectStepInput = (props) => {
  ...
  function handleClearStep() {
    const grantPermission = confirm(
      `Confirm clear ${name} with data : ${JSON.stringify(step)}`
    );
    ...
    <div className="flex flex-col w-full p-4 bg-light drop-shadow-xl filter rounded-md">
      <div className="flex flex-col items-start justify-start w-full">
        <div className="flex items-center justiyf-start gap-x-2">
          {init && Object.values(init).some(Boolean) && (
            <IoCheckmarkDone className="text-sm text-dark" />
          )}
          <span className="text-xs capitalize font-semibold">{name}</span>
        </div>
        {layout.map((el) => (
          <section key={el} className="w-full my-2">
            <StringField name={el} value={step[el]} setValue={handleOnChange} />
          </section>
        ))}
        ...
        {Object.values(step).every((el) => el.trim().length > 0) && (
          <div className="flex items-center justify-around w-full">
            ...
            >
              <span className="py-1.5 px-6 block z-10 peer hover:text-light transition-all hover:shadow-xl border-2 border-dark font-semibold">
                {init && JSON.stringify(init) === JSON.stringify(step)
                  ? `Update ${name}`
                  : `Add ${name}`}
              </span>
              <span className="py-1.5 px-6 block bg-dark transition-all hover:shadow-xl border-2 border-dark absolute top-0 left-0 h-full w-full translate-y-full peer-hover:translate-y-0 z-0 duration-300"></span>
                      ...
                      <summary className="w-full flex justify-between items-center focus:outline-none border-none">
                        <p className="cursor-pointer hover:text-secondary capitalize grow w-full flex items-center justify-start gap-x-2 break-words">
                          {SUMMARY_FIELDS[name] === "hex" && (
                            <span
                              style={{
                                backgroundColor: el[SUMMARY_FIELDS[name]],
                              }}
                              className="h-4 w-4 rounded-full filter drop-shadow-lg block"
                            ></span>
                          )}
                          <span className="font-semibold font-special ">
                            {el[SUMMARY_FIELDS[name]].slice(0, 70)}{" "}
                            {el[SUMMARY_FIELDS[name]].length > 70 && "..."}
                          </span>
                        </p>
                        <button
      ...
      <div className="mt-2 w-full flex flex-col items-start gap-y-2">
        {Object.entries(options).map(([k, v]) => (
          ...
          >
            ...
            />
            {v === value ? (
              <IoCheckmarkCircleSharp className="text-lg text-secondary" />
            ) : (
...
export const MarkdownEditor = ({ init = "", getData }) => {
