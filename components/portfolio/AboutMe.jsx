...
export const AboutMe = ({ isDarkMode, skills = "", techStack = [] }) => {
  return (
    <section
      ...
      className={`min-h-screen mt-40 flex flex-col items-start justify-center w-full snap-start lg:snap-mandatory h-auto mx-auto lg:relative`}
    >
      <div className="px-8 w-full max-w-2xl mx-auto">
        <MarkdownStep text={skills} firstLetter={true} />
      </div>
      <article
        className={`px-8 flex flex-col pt-10 pb-40 lg:py-0 lg:my-20 gap-y-10 after-line--center lg:after:hidden w-full max-w-3xl mx-auto lg:max-w-7xl lg:grid lg:grid-cols-4 h-auto lg:gap-6 skill__card__container`}
      >
        {[...techStack, ...techStack]
          .slice(0, 4)
          .map(({ heading, text }, i) => (
            <Fragment key={i}>
              <section
                className={`p-6 z-10 w-5/6 lg:w-full group lg:hover:scale-105 transition-all will-change-transform skill__card ${
                  i % 2
