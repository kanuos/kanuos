export const StyledHeader = ({
  ...
  return (
    <header className="min-h-screen h-full w-full mx-auto relative block pt-20">
      ...
      >
        <h2
          className={`uppercase absolute z-0 text-4xl sm:text-6xl lg:text-7xl xl:text-9xl font-black bg-contain leading-[0.75] md:leading-[0.75] xl:leading-[0.75] w-min max-w-[17rem] sm:max-w-sm md:max-w-md lg:max-w-xl break-words text-right block box-decoration-clone opacity-30`}
        >
          {styledContent}
        </h2>
        <h2
          className={`styledText uppercase absolute z-10 text-4xl sm:text-6xl lg:text-7xl xl:text-9xl ${
            isDarkMode
              ? "bg-[url('/styledDark.webp')]"
              : "bg-[url('/styledImg.webp')]"
          } opacity-100 font-black bg-contain leading-[0.75] md:leading-[0.75] xl:leading-[0.75] w-min max-w-[17rem] sm:max-w-sm md:max-w-md lg:max-w-xl break-words text-right block box-decoration-clone`}
        >
          {styledContent}
        </h2>
      </div>
