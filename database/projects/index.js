...
export async function projectUniqueConstraint(projectData) {
    ...
    const existingproject = await ProjectModel.findOne({ title : projectData.title });
 ...
 */
 export async function addProjectToDB(projectData) {
    ...
    if (existingproject) throw `project _id:${existingproject._id} already exists with conflicting data`
