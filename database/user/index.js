...
export async function loginAdmin(user) {
  ...
  const isValidCredential = await isValidPassword(
    user.password,
...
export async function getPortfolio() {
  const user = await UserModel.findOne({})
    .select("skills about portfolio techStack location email fullName")
    .populate({
      path: "portfolio",
      populate: {
        path: "design project",
...
export async function addPortfolioProjectToProfile(newDoc) {
  ...
  const populatedData = await PortfolioModel.findById(
    newPortfolio._id
  ).populate({
    path: "design project",
    populate: {
